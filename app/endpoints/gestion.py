...
async def registro_gerente(gerente: Registro, _=Depends(get_current_admin)):
    if await existe_email(gerente.email):
        raise HTTPException(
            detail="email repetido", status_code=status.HTTP_409_CONFLICT
        )
...
async def edita_gerente(
    gerente: UpdateUser, current: Gerente = Depends(get_current_gerente)
):
...
async def solicita_baja_gerente(gerente: Gerente = Depends(get_current_gerente)):
    ...
    if len(responsable.phone_tokens) > 0:
        ...
        message_body = (
            f"El usuario con email {gerente.email} ha solicitado darse de baja."
        )
...
async def get_gerentes(
    salta: int = 0, limite: Optional[int] = None, _=Depends(get_current_admin)
):
