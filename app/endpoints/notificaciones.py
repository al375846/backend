...
async def push(id: ObjectId):
    ...
    if len(tokens) > 0:
        ...
        if notification.tipo_medicion == TipoMedicion.distancia:
            message_title = (
                f"Alerta distancia - {notification.establecimiento.descriptor}"
            )
...
async def listado_notificaciones_admin(
    ...
    listado = await db.motor.find(
        NotificacionAdmin, NotificacionAdmin.responsable == admin.id
    )
...
async def listado_notificaciones_gerente(
    ...
    docs = await noticoll.find(
        {+Notificacion.establecimiento: {"$in": list(dict_establecimientos.keys())}}
    ).to_list(length=None)
    listado = []
    for doc in docs:
...
async def notificacion_leida(
    idn: ObjectId, gerente: Gerente = Depends(get_current_gerente)
):
    notificacion = await db.motor.find_one(Notificacion, Notificacion.id == idn)

    if notificacion is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No existe notificación"
        )

    if notificacion.establecimiento.gerente.id != gerente.id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Notificacion no en propiedad",
        )
